...
var normalizeAddress = (addressToNormalize) => {
  if (addressToNormalize.length > 64 && addressToNormalize.slice(0, 2) === "0x") {
...
var SupraClient = class _SupraClient {
      ...
      Move: {
        ...
        authenticator: {
          Ed25519: {
            ...
            signature: senderAccount.signBuffer(import_aptos.TransactionBuilder.getSigningMessage(rawTxn)).toString()
          }
        }
      }
    };
  ...
  async getTxObject(senderAddr, moduleAddr, moduleName, functionName, functionTypeArgs, functionArgs, maxGas = BigInt(2e3)) {
    return new import_aptos.TxnBuilderTypes.RawTransaction(
      ...
      new import_aptos.TxnBuilderTypes.TransactionPayloadEntryFunction(
        new import_aptos.TxnBuilderTypes.EntryFunction(
          new import_aptos.TxnBuilderTypes.ModuleId(
            new import_aptos.TxnBuilderTypes.AccountAddress(
              new import_aptos.HexString(normalizeAddress(moduleAddr)).toUint8Array()
            ),
            new import_aptos.TxnBuilderTypes.Identifier(moduleName)
          ),
          new import_aptos.TxnBuilderTypes.Identifier(functionName),
          functionTypeArgs,
          functionArgs
        )
      ),
      maxGas,
      // Setting MaxGasAmount As 2000 Because In Devnet Only Those Transactions Will Be Executing Using This Method Whose Gas Consumption Will Always Less Than 2000
   ...
   */
  async transferSupraCoinPayload(senderAccount, receiverAccountAddr, amount) {
    let maxGas = BigInt(10);
    if (await this.isAccountExists(receiverAccountAddr) == false) {
      maxGas = BigInt(1020);
    }
    if (amount + maxGas * BigInt(100) > await this.getAccountSupraCoinBalance(senderAccount.address())) {
  ...
  async publishPackage(senderAccount, packageMetadata, modulesCode) {
    let codeSerializer = new import_aptos.BCS.Serializer();
    let modulesTypeCode = [];
    for (let i = 0; i < modulesCode.length; i++) {
      modulesTypeCode.push(
        new import_aptos.TxnBuilderTypes.Module(Uint8Array.from(modulesCode[i]))
      );
    }
    import_aptos.BCS.serializeVector(modulesTypeCode, codeSerializer);
    let sendTxPayload = await this.getSendTxPayload(
      senderAccount,
      await this.getTxObject(
        senderAccount.address(),
